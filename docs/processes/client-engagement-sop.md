...
```mermaid
flowchart LR
    P1[Phase 1<br/>Prospect] --> P2[Phase 2<br/>Discovery]
    P2 --> P3[Phase 3<br/>Proposal]
    P3 --> P4[Phase 4<br/>Close]
    P4 --> P5[Phase 5<br/>Deliver]
    P5 --> P6[Phase 6<br/>Expand]
    P6 --> P7[Phase 7<br/>Compound]
    P7 -.->|Referrals| P1
    P7 -.->|Upsell| P3
```
...
```mermaid
flowchart TD
    CONNECT[CONNECT<br/>Build rapport, reference context] --> ASK[ASK<br/>Deep-dive into their situation]
    ASK --> QUANTIFY[QUANTIFY<br/>Put numbers on the pain]
    QUANTIFY --> OFFER[OFFER<br/>Present the diagnostic engagement]

    ASK --> Q1[What is the core challenge?]
    ASK --> Q2[How long has this existed?]
    ASK --> Q3[What have you tried?]
    ASK --> Q4[What is the impact on revenue/operations?]

    QUANTIFY --> N1[Revenue at risk?]
    QUANTIFY --> N2[Cost of inaction per month?]
    QUANTIFY --> N3[Opportunity cost?]
    QUANTIFY --> N4[Regulatory/compliance exposure?]

    OFFER --> O1[Diagnostic engagement: $5-15K]
    OFFER --> O2[Timeline: 5-10 business days]
    OFFER --> O3[Deliverable: Findings report + chokepoint map]
    OFFER --> O4[Guarantee: 3x value or refund]
```
...
```mermaid
flowchart LR
    COMM[Commercial Operator] -->|Handoff Package| EXEC[Execution Operator]

    subgraph HANDOFF[Handoff Package Contents]
        H1[Discovery notes + pain quantification]
        H2[Signed proposal + contract]
        H3[Client stakeholder map]
        H4[Access requirements]
        H5[Risk factors and sensitivities]
    end
```
...
```mermaid
gantt
    title Diagnostic Sprint Timeline
    dateFormat YYYY-MM-DD
    axisFormat %A

    section Week 1
    Kickoff + Data Collection      :d1, 2025-01-06, 2d
    Stakeholder Interviews         :d2, after d1, 2d
    Process Mapping                :d3, after d2, 1d

    section Week 2
    Analysis + Chokepoint ID       :d4, 2025-01-13, 2d
    Findings Report Drafting       :d5, after d4, 1d
    Internal Review                :d6, after d5, 1d
    Client Presentation            :d7, after d6, 1d
```
...
```mermaid
flowchart TD
    CLIENT([Existing Client — Compounding Phase])
    CLIENT --> SAT{Client satisfaction high?}
    SAT -->|Yes| UPSELL[Propose higher-tier engagement]
    SAT -->|No| REMEDIATE[Address dissatisfaction first]

    UPSELL --> PIAR[PIAR Framework<br/>$8-20K]
    UPSELL --> ENT[Enterprise Engagement<br/>$50K+]
    UPSELL --> GOV[Governance Design<br/>$15-30K]

    PIAR --> COMPOUND[Case study + referral + next engagement]
    ENT --> COMPOUND
    GOV --> COMPOUND
    COMPOUND -.->|Referrals feed pipeline| PROSPECT([Phase 1: New Prospects])
```
...
```mermaid
flowchart TD
    subgraph Phase1to2[Phase 1 → 2: Prospect to Discovery]
        A1[Commercial Operator retains ownership]
        A2[No handoff — same owner]
    end

    subgraph Phase4to5[Phase 4 → 5: Close to Deliver]
        B1[Commercial Operator → Execution Operator]
        B2[Handoff package: discovery notes, contract, stakeholder map]
        B3[Joint kickoff call]
    end

    subgraph Phase5to6[Phase 5 → 6: Deliver to Expand]
        C1[Execution Operator → Commercial Operator]
        C2[Handoff: findings, client feedback, expansion recommendations]
        C3[Joint expansion strategy session]
    end

    subgraph Phase6to7[Phase 6 → 7: Expand to Compound]
        D1[Commercial Operator retains ownership]
        D2[Cell Lead involved for strategic accounts]
    end
```
