...
description: "The cognitive foundations, thinking tools, knowledge containers, belief systems, and learning taxonomies that form the raw material from which all AINEFF systems, governance, and execution derive."
...
```mermaid
flowchart LR
    subgraph Substrate["Knowledge Substrate (This Section)"]
        direction TB
        TT["Thinking Tools"]
        KC["Knowledge Containers"]
        BS["Belief Systems"]
        LD["Learning Domains"]
        HP["Human Progression"]
    end

    subgraph Processing["Cognitive Processing"]
        direction TB
        GRIL["GRIL — Grounded Retrieval"]
        CPE["CPE — Cognitive Post-Processing"]
        OMG["OMG — Orchestrated Memory"]
    end

    subgraph Application["Operational Systems"]
        direction TB
        GKMS["GKMS — Governance Knowledge"]
        KIMS["KIMS — Knowledge Integrity"]
        WGE["WGE — Work Genesis Engine"]
    end

    Substrate -->|"Raw material"| Processing
    Processing -->|"Structured intelligence"| Application

    style Substrate fill:#1a1a3a,stroke:#6a6aba,color:#fff
    style Processing fill:#1a3a1a,stroke:#6aba6a,color:#fff
    style Application fill:#3a1a1a,stroke:#ba6a6a,color:#fff
```
