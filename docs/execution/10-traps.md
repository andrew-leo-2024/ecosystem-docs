...
```mermaid
flowchart TD
    subgraph TRAPS["The 10 Traps (DO NOT DO)"]
        T1["1. Build a Product"]
        T2["2. Write Strategy Docs"]
        T3["3. Design a Logo"]
        T4["4. Build 74-System Architecture"]
        T5["5. Incorporate in Multiple Jurisdictions"]
        T6["6. Attend Conferences"]
        T7["7. Hire Anyone"]
        T8["8. Raise Capital"]
        T9["9. Build an App"]
        T10["10. Think About ORF"]
    end

    subgraph DO["INSTEAD: Do This"]
        D1["Open LinkedIn"]
        D2["Find one COO at a $10M company"]
        D3["Send one message"]
    end

    TRAPS -->|"Replace with"| DO

    style TRAPS fill:#3a1a1a,stroke:#ba4a4a,color:#fff
    style DO fill:#1a3a1a,stroke:#4aba4a,color:#fff
```
...
```mermaid
flowchart TD
    TRAP["Caught doing a trap"] --> STOP["STOP immediately"]
    STOP --> OPEN["Open LinkedIn"]
    OPEN --> FIND["Find one COO at a $10M company"]
    FIND --> SEND["Send one personalized message"]
    SEND --> WAIT["Wait for response"]
    WAIT --> REPEAT["Repeat"]

    style TRAP fill:#3a1a1a,stroke:#ba4a4a,color:#fff
    style STOP fill:#3a2a1a,stroke:#ba6a4a,color:#fff
    style OPEN fill:#2a3a1a,stroke:#6aba4a,color:#fff
    style FIND fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style SEND fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style WAIT fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style REPEAT fill:#1a3a1a,stroke:#4aba4a,color:#fff
```
