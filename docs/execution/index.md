...
```mermaid
flowchart TD
    subgraph DOCTRINE["Execution Doctrine"]
        direction TB
        R["REVENUE FIRST
        Nothing matters until
        money arrives"]
        C["CONVERSATION DRIVEN
        Every deal starts with
        a human conversation"]
        S["SEQUENTIAL, NOT PARALLEL
        One phase at a time.
        No skipping."]
        D["DISCIPLINE OVER TALENT
        Systems beat heroes.
        ...
        Redirect resources."]
        M["MEASURE WEEKLY
        KPIs with red/yellow/green.
        No vanity metrics."]
        A["AUTOMATE LAST
        Prove manually first.
        Then automate the proven."]
    end

    R --> C --> S --> D --> K --> M --> A

    style DOCTRINE fill:#1a1a2e,stroke:#4a9aba,color:#fff
    style R fill:#2a1a1a,stroke:#ba4a4a,color:#fff
    style C fill:#1a2a1a,stroke:#4aba4a,color:#fff
    style S fill:#1a1a2a,stroke:#4a4aba,color:#fff
    style D fill:#2a2a1a,stroke:#baba4a,color:#fff
    style K fill:#2a1a2a,stroke:#ba4aba,color:#fff
    style M fill:#1a2a2a,stroke:#4ababa,color:#fff
    style A fill:#2a1a1a,stroke:#ba6a4a,color:#fff
```
...
```mermaid
graph LR
    subgraph WRONG["Ambitious Parallelism (WRONG)"]
        direction TB
        W1["Product Dev"] ~~~ W2["Sales"]
        W3["Platform"] ~~~ W4["Hiring"]
        W5["Fundraising"] ~~~ W6["Marketing"]
    end

    subgraph RIGHT["Sequential Discipline (RIGHT)"]
        direction LR
        R1["Conversations"] --> R2["First Deal"]
        R2 --> R3["First Revenue"]
        R3 --> R4["Repeat"]
        R4 --> R5["Systemize"]
        R5 --> R6["Scale"]
    ...
    style WRONG fill:#3a1a1a,stroke:#ba4a4a,color:#fff
    style RIGHT fill:#1a3a1a,stroke:#4aba4a,color:#fff
```
