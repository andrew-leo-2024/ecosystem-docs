...
```mermaid
flowchart LR
    I1["1. The Grand Vision
    (Why this exists)"] --> I2["2. Centi-Trillion Thesis
    (Scale ambition)"]
    I2 --> I3["3. Financial Model
    (3-year projections)"]
    I3 --> I4["4. Revenue Streams
    (25+ revenue channels)"]
    I4 --> I5["5. Unit Economics
    (Venture cell economics)"]
    I5 --> I6["6. Capital Strategy
    (Funding approach)"]
    I6 --> I7["7. 7-Phase Progression
    (Execution roadmap)"]
    I7 --> I8["8. Compounding Leverage
    (7 asset classes)"]

    style I1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style I8 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
```mermaid
flowchart LR
    R1["1. Regulator Brief
    (Executive summary)"] --> R2["2. Atomic Constraint
    (Core principle)"]
    R2 --> R3["3. Core DNA
    (18 principles)"]
    R3 --> R4["4. AINEFF
    (Constitutional layer)"]
    R4 --> R5["5. Anti-ASI Clause
    (Power concentration limits)"]
    R5 --> R6["6. Audit & Evidence
    (Evidence architecture)"]
    R6 --> R7["7. External Oversight
    (Regulator interfaces)"]

    style R1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style R7 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
```mermaid
flowchart LR
    E1["1. Enterprise Wedge
    (90-day proof engine)"] --> E2["2. 30-Day Plan
    (Engagement mechanics)"]
    E2 --> E3["3. Chokepoint Intelligence
    (Bottleneck mapping)"]
    E3 --> E4["4. Billing Leakage
    (Revenue recovery)"]
    E4 --> E5["5. Insurance Claims
    (Claims automation)"]
    E5 --> E6["6. Pricing Architecture
    (How you pay)"]

    style E1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style E6 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
flowchart LR
    O1["1. LevelUpMax
    (Talent refinery)"] --> O2["2. M-Shaped Mind
    (Cognitive model)"]
    O2 --> O3["3. Operator Onboarding
    (6-stage SOP)"]
    O3 --> O4["4. Venture Cell SOP
    (Daily operations)"]
    O4 --> O5["5. Operator Training
    (Track 1-5 bootcamp)"]

    style O1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style O5 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
```mermaid
flowchart LR
    T1["1. Architecture Overview
    (Full stack model)"] --> T2["2. Agent Taxonomy
    (Agent classification)"]
    T2 --> T3["3. Protocol Architecture
    (PCP / PEP)"]
    T3 --> T4["4. EMS Architecture
    (Enterprise manufacturing)"]
    T4 --> T5["5. Memory Systems
    (OMG and knowledge)"]
    T5 --> T6["6. Skills Architecture
    (Skill composition)"]
    T6 --> T7["7. Deployment Infrastructure
    (CI/CD and runtime)"]

    style T1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style T7 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
flowchart LR
    G1["1. Frankmax
    (Governance entity)"] --> G2["2. PIAR
    (Pre-Incident Review)"]
    G2 --> G3["3. 15 Systems
    (Coordination substrate)"]
    G3 --> G4["4. Kill Discipline
    (When to stop)"]
    G4 --> G5["5. Governance Review SOP
    (Rule change procedures)"]
    G5 --> G6["6. Audit SOP
    (Audit procedures)"]

    style G1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style G6 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
```mermaid
flowchart LR
    F1["1. Why This Exists
    (Origin and purpose)"] --> F2["2. Inevitability Framework
    (Why it must happen)"]
    F2 --> F3["3. 7-Phase Progression
    (Execution roadmap)"]
    F3 --> F4["4. Founder Discipline
    (What founders must do)"]
    F4 --> F5["5. The 10 Traps
    (What NOT to do)"]
    F5 --> F6["6. Compounding Leverage
    (7 asset classes)"]

    style F1 fill:#1a3a1a,stroke:#4aba4a,color:#fff
    style F6 fill:#1a1a3a,stroke:#4a4aba,color:#fff
```
...
```mermaid
flowchart TB
    subgraph PHASE1["Phase 1: Vision & Philosophy (2-3 hrs)"]
        direction LR
        D1["Vision Overview"]
        D2["Why This Exists"]
        D3["Atomic Constraint"]
        D4["Core DNA"]
        D5["Inevitability"]
        D6["Centi-Trillion Thesis"]
        D7["Compounding Thesis"]
    end

    subgraph PHASE2["Phase 2: Blueprint & Structure (2-3 hrs)"]
        direction LR
        D8["Blueprint Overview"]
        D9["Entity Hierarchy"]
        D10["Three Domains"]
        D11["15 Systems"]
        D12["7-Layer Control"]
        D13["5-Layer Monopoly"]
        D14["22 Platforms"]
        D15["Energy-Capital-Information"]
    end

    subgraph PHASE3["Phase 3: Entities & Platforms (2-3 hrs)"]
        direction LR
        D16["Entities Overview"]
        D17["AINEFF"]
        D18["AINEF OS"]
        D19["AINEG"]
        D20["AINE"]
        ...
        D23["LevelUpMax"]
        D24["ORF Protocol"]
        D25["Ecosystem Brands"]
        D26["Regulator Brief"]
    end

    subgraph PHASE4["Phase 4: Systems & Modules (3-4 hrs)"]
        direction LR
        D27["Systems Overview"]
        D28["21 Core Systems"]
        D29["Framework Systems"]
        D30["Factory Systems"]
        D31["Group Systems"]
        D32["Enterprise Systems"]
        D33["External Oversight"]
        D34["Skills Systems"]
        D35["Frankmax 15 Systems"]
        D36["Factory Byproducts"]
    end

    subgraph PHASE5["Phase 5: Products & Revenue (2-3 hrs)"]
        direction LR
        D37["Products Overview"]
        D38["Revenue Streams"]
        D39["Market Wedges"]
        D40["All 8 Product Offerings"]
        D41["Insurance Vertical"]
        D42["Pricing Architecture"]
        D43["Unit Economics"]
        D44["Enterprise Wedge"]
        D45["Monetization Boundaries"]
    end

    subgraph PHASE6["Phase 6: Execution (2-3 hrs)"]
        direction LR
        D46["7-Phase Progression"]
        D47["30-Day Plan"]
        D48["90-Day Sprint"]
        D49["Kill Discipline"]
        D50["Financial Model"]
        D51["Capital Strategy"]
        D52["Compounding Leverage"]
        D53["Founder Discipline"]
        D54["10 Traps"]
        D55["Deployment Sequencing"]
        D56["KPI Dashboard"]
        D57["Risk Register"]
        D58["Conversation Framework"]
    end

    subgraph PHASE7["Phase 7: Architecture (3-4 hrs)"]
        direction LR
        D59["Architecture Overview"]
        D60["Agent Taxonomy"]
        D61["Agent Design Patterns"]
        D62["Protocol Architecture"]
        D63["EMS Architecture"]
        D64["Memory Systems"]
        D65["Skills Architecture"]
        D66["Audit & Evidence"]
        D67["Governance Enforcement"]
        D68["Application Architecture"]
        D69["Signal Harvesters"]
        D70["AI Taxonomy"]
        D71["Deployment Infrastructure"]
    end

    subgraph PHASE8["Phase 8: Knowledge & Processes (2-3 hrs)"]
        direction LR
        D72["Knowledge Overview"]
        D73["M-Shaped Mind"]
        D74["E2E Progression"]
        D75["Learning Categories"]
        D76["Knowledge Containers"]
        D77["Thinking Tools"]
        D78["Agentic UX"]
        D79["Belief Systems"]
        D80["Processes Overview"]
        D81["All SOPs"]
    end

    PHASE1 --> PHASE2 --> PHASE3 --> PHASE4 --> PHASE5 --> PHASE6 --> PHASE7 --> PHASE8

    style PHASE1 fill:#1a1a3a,stroke:#6a6aba,color:#fff
    style PHASE2 fill:#1a3a3a,stroke:#4ababa,color:#fff
    style PHASE3 fill:#1a3a1a,stroke:#4aba6a,color:#fff
    style PHASE4 fill:#2a2a1a,stroke:#baba4a,color:#fff
    style PHASE5 fill:#3a2a1a,stroke:#ba8a4a,color:#fff
    style PHASE6 fill:#3a1a1a,stroke:#ba6a6a,color:#fff
    style PHASE7 fill:#2a1a3a,stroke:#8a4aba,color:#fff
    style PHASE8 fill:#3a1a3a,stroke:#ba4aba,color:#fff
```
