...
```mermaid
flowchart TD
    A["Violation Detected<br/>(PAME/ICG/GAA)"] --> B{"Severity?"}
    B -->|"Potential/Minor"| C["Level 1: Signal Freeze<br/>Revenue stream frozen"]
    B -->|"Confirmed Minor"| D["Level 2: Public Mark<br/>Entity flagged"]
    B -->|"Confirmed Moderate"| E["Level 3: Capability Stripping<br/>Authorities removed"]
    B -->|"Systemic"| F["Level 4: Forced Unbundling<br/>Entity restructured"]
    B -->|"Severe/Repeated"| G["Level 5: Ejection<br/>Entity removed"]

    C --> H{"Investigation<br/>Result?"}
    H -->|"Cleared"| I["Unfreeze + Record"]
    H -->|"Confirmed"| D

    D --> J{"Remediation<br/>within 90 days?"}
    J -->|"Yes"| K["Mark scheduled for removal"]
    J -->|"No"| E

    E --> L{"Remediation<br/>within 180 days?"}
    L -->|"Yes"| M["Capabilities restored after review"]
    L -->|"No"| F

    F --> N{"Compliance<br/>within 12 months?"}
    N -->|"Yes"| O["Monitor with enhanced oversight"]
    N -->|"No"| G
```
