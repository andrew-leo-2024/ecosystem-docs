...
```mermaid
pie title Single Venture Cell Cost Allocation ($100K Revenue)
    "Gross Margin" : 52
    "Commercial" : 12
    "Execution" : 12
    "Cell Lead" : 18
...
```mermaid
flowchart TD
    A["Kill Criterion Breached"] --> B{"Which criterion?"}
    B -->|"Margin <40%"| C["Immediate cost review"]
    B -->|"<6 customers"| D["Pipeline and sales audit"]
    B -->|"Unpaid invoices"| E["Collection action"]
    B -->|"NPS/complaints"| F["Quality review"]
    B -->|"Revenue decline"| G["Marketing intervention"]
    B -->|"Team turnover"| H["Leadership assessment"]

    C --> I["30-day remediation plan"]
    D --> I
    E --> J{"Collected<br/>within 30 days?"}
    F --> I
    G --> I
    H --> I

    J -->|"Yes"| K["Monitor"]
    J -->|"No"| L["Client termination + write-off"]

    I --> M{"Remediated<br/>within 30 days?"}
    M -->|"Yes"| N["Return to normal + enhanced monitoring"]
    M -->|"No"| O["Cell dissolution<br/>Resources redeployed"]
```
