...
```mermaid
flowchart LR
    A["Approval<br/>Decision authorization"] --> B["Execution<br/>Work performance"]
    B --> C["Delivery<br/>Output transfer"]
    C --> D["Payment<br/>Revenue collection"]
    D -->|"Next cycle"| A

    A1["Chokepoint A<br/>Approval delays"] -.-> A
    B1["Chokepoint B<br/>Execution bottlenecks"] -.-> B
    C1["Chokepoint C<br/>Delivery friction"] -.-> C
    D1["Chokepoint D<br/>Payment collection"] -.-> D

    style A1 fill:#5a2d2d,stroke:#f87171
    style B1 fill:#5a2d2d,stroke:#f87171
    style C1 fill:#5a2d2d,stroke:#f87171
    style D1 fill:#5a2d2d,stroke:#f87171
```
