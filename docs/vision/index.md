...
graph LR
    ...
    'Nice to have'"] --> B["Phase 2: Essential
    ...
    style A fill:#1a3a4a,stroke:#4a9aba,color:#fff
    style B fill:#2a4a3a,stroke:#4aba6a,color:#fff
    style C fill:#4a3a1a,stroke:#baba4a,color:#fff
```
...
```mermaid
flowchart TB
    subgraph AUTHORITY["Authority Layer (andrewfranklinleo.com)"]
    ...
    subgraph EXECUTION["Execution Layer (frankmax.com)"]
        DIAG["Diagnostic
        Products"]
        ORCH["Orchestration
        Engines"]
    ...
    subgraph INFRASTRUCTURE["Infrastructure Layer (Protocol)"]
        OBL["Obligation
        Registry"]
        AUDIT["Audit
        Substrate"]
    ...
    subgraph MARKET["Market Adoption"]
        ENT["Enterprise
        Clients"]
        INS["Insurance
        Carriers"]
    ...
    DIAG -->|"Feeds data to"| OBL
    ORCH -->|"Routes through"| COORD
    OPS -->|"Reports to"| AUDIT

    OBL --> ENT
    AUDIT --> INS
    COORD --> REG

    ENT -->|"Revenue"| EXECUTION
    INS -->|"Mandates"| EXECUTION
    REG -->|"Legitimizes"| AUTHORITY

    style AUTHORITY fill:#1a1a3a,stroke:#6a6aba,color:#fff
    style EXECUTION fill:#1a3a1a,stroke:#6aba6a,color:#fff
    style INFRASTRUCTURE fill:#3a1a1a,stroke:#ba6a6a,color:#fff
    style MARKET fill:#3a3a1a,stroke:#baba6a,color:#fff
```
