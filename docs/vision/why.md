...
```mermaid
graph TB
    subgraph GAP["The Widening Gap"]
        direction LR
        TECH["Technology Capability
        (Exponential)"]
        SOC["Social Contract Adaptation
        (Generational)"]
    end

    TECH --> T1["AI can make
    autonomous decisions"]
    SOC --> S1["Legal frameworks still assume
    humans make all decisions"]

    TECH --> T2["Global coordination
    is instant"]
    SOC --> S2["Governance is still
    jurisdictional"]

    TECH --> T3["Algorithmic systems
    manage billions"]
    SOC --> S3["Accountability frameworks
    assume individual actors"]

    T1 -.- S1
    T2 -.- S2
    T3 -.- S3

    style GAP fill:#3a1a1a,stroke:#ba6a6a,color:#fff
    style TECH fill:#1a3a1a,stroke:#6aba6a,color:#fff
    style SOC fill:#1a1a3a,stroke:#6a6aba,color:#fff
```
...
graph TD
    A["Small erosion
    (barely noticeable)"] --> B["Normalization
    ('this is just how things are')"]
    B --> C["Adaptation
    (lower expectations)"]
    C --> D["Forgetting
    (what it was supposed to be)"]
    D --> E["Structural lock-in
    (too late to change cheaply)"]
    E --> F["Crisis
    (the bill comes due)"]

    style A fill:#2a3a1a,stroke:#6a8a4a,color:#fff
    style B fill:#3a3a1a,stroke:#8a8a4a,color:#fff
    style C fill:#3a2a1a,stroke:#8a6a4a,color:#fff
    style D fill:#3a1a1a,stroke:#8a4a4a,color:#fff
    style E fill:#4a1a1a,stroke:#aa4a4a,color:#fff
    style F fill:#5a1a1a,stroke:#ca4a4a,color:#fff
```
